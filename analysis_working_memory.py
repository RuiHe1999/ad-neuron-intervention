...
# Working memory: convert each score to error, then sum the four errors.
wm_vars = list(wm_files.keys())
error_vars = [f"{v}_error" for v in wm_vars]
...
def pearson_gof(model):
    chi2_val = float(np.sum(model.resid_pearson ** 2))
    df = float(model.df_resid)
    if df <= 0:
        return chi2_val, df, np.nan, np.nan
    return chi2_val, df, chi2_val / df, float(stats.chi2.sf(chi2_val, df))
...
def plot_one_variable(ax, data, glm_res, var):
    ...
    plot_dat = data[["Condition", "alpha_signed", "neurons", var]].copy()
    ...
    if not glm_res.empty:
        sig_dat = glm_res[glm_res["var"] == var][["condition", "p_adj"]].copy()
...
wm_paths = {var: os.path.join("Chat_analysis/annotations", filename) for var, filename in wm_files.items()}
...
variables = [v for v in variables if v in wm_res.columns]

# 4.3 GLM
if {"Role", "Condition"}.issubset(wm_res.columns):
    glm_res = []
    ...
    for var in variables:
        ...
        formula = f'Q("{var}") ~ C(Role) + C(Condition, Treatment(reference="Original"))'
        res = smf.glm(formula=formula, data=dat, family=family).fit()
        pearson_chi2, pearson_df, pearson_ratio, pearson_p = pearson_gof(res)
        ...
        glm_res.append(pd.DataFrame(rows))

    glm_res = pd.concat(glm_res, ignore_index=True) if len(glm_res) > 0 else pd.DataFrame()

    if not glm_res.empty:
        glm_res["p_adj"] = np.nan
        for var in glm_res["var"].dropna().unique():
            idx = glm_res["var"] == var
            glm_res.loc[idx, "p_adj"] = pg.multicomp(glm_res.loc[idx, "p"].values, method="fdr_bh")[1]

        glm_res.to_csv(
            os.path.join(wm_output, "glm_condition_vs_original.csv"),
            index=False,
            encoding="utf-8-sig",
        )
else:
...
# 4.4 Visualization
if {"Condition", "alpha_signed", "neurons"}.issubset(wm_res.columns) and len(variables) > 0:
    ...
    for ax, var in zip(axes, error_vars):
        plot_one_variable(ax, wm_res, glm_res, var)

    plot_one_variable(total_ax, wm_res, glm_res, total_var)
